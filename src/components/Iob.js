...
*/
  ...
  static getHome(link) {
    const { host, protocol, adapterName } = Iob.getStore.location;
    return `${protocol}//${host}${link ? link : "/adapter/" + adapterName + "/"}`;
  }
  ...
  static getObject(id) {
    ...
    Iob.connection.getObject(id).then(
      (o) => {
        ...
        Iob.emitEvent("objectChange", obj);
        if (obj.id == Iob.getStore.instanceId) Iob.setInstanceConfig(obj.newObj);
